...
void user_process(){
	char buf[30];
	sprintf(buf, "User process started. EL\n\r");
	call_sys_write(buf);
	...
	sprintf(buf, "About to clone proc\r\n");
	call_sys_write(buf);
	...
	sprintf(buf, "Cloned proc1 again\r\n");
	call_sys_write(buf); 
	call_sys_exit();
}
...
void kernel_main(void)
{
	...
	int res = copy_process(PF_KTHREAD, (unsigned long)&kernel_process, 0, 0, "kernel-main");
	if (res < 0) {
